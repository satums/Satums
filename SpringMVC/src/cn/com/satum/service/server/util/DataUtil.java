...
public  String dataUpdate(String zjbhs, String iPs, int ports,String dates){
	...
	String message="S";	
	if(list.size()>0){
		//System.out.println("-------------===================update"+"update sh_onlinedata set updated_date='"+dates+"',IP='"+iPs+"',port='"+ports+"' where zjbh='"+zjbhs+"'");
		appBo.runSQL("update sh_onlinedata set updated_date='"+dates+"',IP='"+iPs+"',port='"+ports+"' where zjbh='"+zjbhs+"'");
	}else{
		...
		try{
			String sql="INSERT INTO sh_onlinedata VALUES ('"+(id+1)+"','"+zjbhs+"','"+iPs+"','"+ports+"','"+dates+"','"+dates+"','0','','')";
			System.out.println(sql);
			appBo.runSQL("INSERT INTO sh_onlinedata VALUES ('"+(id+1)+"','"+zjbhs+"','"+iPs+"','"+ports+"','"+dates+"','"+dates+"','0','','')");
		}catch(Exception e){
...
public  Map dataQuery(String zjbhs,String jsonData) throws ParseException{
	...
	if(listu.size()>0){	
	 ...
	 String userid="15811137696";
		...
		appBo.runSQL("INSERT INTO sh_masterdata VALUES ('"+(id+1)+"','"+userid+"','"+zjbhs+"','"+jsonData+"','0','','')");
	}
	 SimpleDateFormat sdf =   new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss" );	 
	Map map=null;
